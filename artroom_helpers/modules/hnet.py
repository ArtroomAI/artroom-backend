...
class Hypernetwork(torch.nn.Module):
    def __init__(self, state_dict) -> None:
        super().__init__()
        self.dropout_structure = None
        self.use_dropout = None
        self.last_layer_dropout = None
        self.activate_output = None
        self.add_layer_norm = None
    ...
    def build_model(self, state_dict):
        ...
        self.use_dropout = True if self.dropout_structure is not None and any(
            self.dropout_structure) else state_dict.get('use_dropout', False)
        ...
        for size in self.sizes:
            self.layers[size] = (
                HypernetworkModule(size, self.layer_structure, self.activation_func, self.add_layer_norm,
                                   self.activate_output, self.dropout_structure),
                HypernetworkModule(size, self.layer_structure, self.activation_func, self.add_layer_norm,
                                   self.activate_output, self.dropout_structure)
            )
